;
; i2cset.asm -- Command line I2C write byte tool 
;
; Copyright (C) 2007 - 2009, Merck Hung
; Author : Merck Hung <merckhung@gmail.com>
;
; This tool was made for MS-DOS command line write byte from user specified
...
main PROC FAR
    ...
    ;
    mov     al, slave_addr
    replace_ascii_str   writemsg, 2, 30

    mov     al, data_value
    replace_ascii_str   writemsg, 2, 63

    mov     al, reg_offset
    replace_ascii_str   writemsg, 2, 49
    ...
    ;
    mov     al, slave_addr
    replace_ascii_str   resultmsg, 2, 26
    mov     ah, slave_addr

    mov     al, reg_offset
    replace_ascii_str   resultmsg, 2, 45

    call    i2c_byte_read
    replace_ascii_str   resultmsg, 2, 59
...
version     db      13, 10, 'i2cset version 0.2 (C) 2007, Merck Hung', 13, 10, 13, 10, 24h
...
usage       db      'Usage: i2cget SLAVE_ADDR REG_OFFSET DATA_VALUE', 13, 10, 13, 10
            ...
            db      '   REG_OFFSET: Register offset in I2C device    (0x00 - 0xff)', 13, 10
            db      '   DATA_VALUE: Value to write                   (0x00 - 0xff)', 13, 10, 13, 10, 24h
...
writemsg    db      13, 10, 13, 10, 'Write  I2C slave addr = 0x??, reg offset = 0x??, value = 0x??', 13, 10, 24h
;                                                             30                 49            63

resultmsg   db                      'Verify I2C slave addr = 0x??, reg offset = 0x??, value = 0x??', 13, 10, 24h
;                                                             26                 45            59
