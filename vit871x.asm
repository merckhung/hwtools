...
; Only view IT871x Environment Controller registers, make no any changes
;
.586P
...
it871x_chipfound:
    ...
    ;sio_debug_io_reg 'Enable FANs SMI# idx=0x04', it871x_ec_ioaddr, 004h
    ;sio_debug_io_reg 'Enable VOLT SMI# idx=0x05', it871x_ec_ioaddr, 005h
    ;sio_debug_io_reg 'Enable TEMP SMI# idx=0x06', it871x_ec_ioaddr, 006h
...
it871x_interval PROC NEAR PRIVATE
    ...
    ; Get current FAN1
    sio_io_read_reg it871x_ec_ioaddr, 0dh
    replace_ascii_str fan_msg, 2, 16

    ; Get current FAN2
    sio_io_read_reg it871x_ec_ioaddr, 0eh
    replace_ascii_str fan_msg, 2, 29

    ; Get current FAN3
    sio_io_read_reg it871x_ec_ioaddr, 0fh
    replace_ascii_str fan_msg, 2, 42



    ; Get current FAN1 Limit
    sio_io_read_reg it871x_ec_ioaddr, 010h
    replace_ascii_str fan_lim_msg, 2, 16

    ; Get current FAN2 Limit
    sio_io_read_reg it871x_ec_ioaddr, 011h
    replace_ascii_str fan_lim_msg, 2, 29

    ; Get current FAN3 Limit
    sio_io_read_reg it871x_ec_ioaddr, 012h
    replace_ascii_str fan_lim_msg, 2, 42
    ...
    ; Get SMI# enable status 0x04
    sio_io_read_reg it871x_ec_ioaddr, 004h
    replace_ascii_str en_smi_msg, 2, 23


    ; Get SMI# enable status 0x05
    sio_io_read_reg it871x_ec_ioaddr, 005h
    replace_ascii_str en_smi_msg, 2, 37


    ; Get SMI# enable status 0x06
    sio_io_read_reg it871x_ec_ioaddr, 006h
    replace_ascii_str en_smi_msg, 2, 51
    ...
    ; Get SMI# control 1
    sio_read_reg SIO_CSMI1_ADD
    replace_ascii_str smi_cnt_msg, 2, 23


    ; Get SMI# control 2
    sio_read_reg SIO_CSMI2_ADD
    replace_ascii_str smi_cnt_msg, 2, 30


    ; Get SMI# status 1
    sio_read_reg SIO_SSMI1_ADD
    replace_ascii_str smi_sts_msg, 2, 25


    ; Get SMI# status 2
    sio_read_reg SIO_SSMI2_ADD
    replace_ascii_str smi_sts_msg, 2, 32
...
temp3_msg   DB      'Temperature3 current = 0x??h, max = 0x??h, min = 0x??h', 13, 10
fan_msg     DB      'Current FAN1: 0x??h, FAN2: 0x??h, FAN3: 0x??h', 13, 10
fan_lim_msg DB      'Limit   FAN1: 0x??h, FAN2: 0x??h, FAN3: 0x??h', 13, 10
ec_int1_msg DB      'EC Interrupt Status Register 1: 0x??h', 13, 10
ec_int2_msg DB      'EC Interrupt Status Register 2: 0x??h', 13, 10
ec_int3_msg DB      'EC Interrupt Status Register 3: 0x??h', 13, 10
en_smi_msg  DB      'Enable SMI# : 0x04 = 0x??h, 0x05 = 0x??h, 0x06 = 0x??h', 13, 10
smi_cnt_msg DB      'SMI# Ctrl Registers: 0x??h, 0x??h', 13, 10
smi_sts_msg DB      'SMI# Status Registers: 0x??h, 0x??h', 13, 10, 24h
